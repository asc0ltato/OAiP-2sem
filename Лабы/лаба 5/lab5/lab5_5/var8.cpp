...
//считывание списка из файла. Создать список, содержащий элементы целого типа. Найти сумму отрицательных элементов, кратных 2, или выдать сообщение,
...
void searchList(list* p, int value); // поиск элемента
void toFile(list*& p);         //Запись в файл
void fromFile(list*& p);       //Считывание из файла
void menu(void);     //функция, показывающая меню
void sumOfNegativeMultiplesOfTwo(list*);  //функция суммы отрицательных элементов, кратных 2
...
int main()
{
	...
	while (choice != 8)
	{
		...
		case 6:  fromFile(first);
			break;
		case 7:   sumOfNegativeMultiplesOfTwo(first);	// вычисление суммы	
			break;
...
void sumOfNegativeMultiplesOfTwo(list* p)
{
	int sm = 0;
	if (p == NULL) {
		cout << "Список пуст" << endl;
	...
	else
	{
		...
		if (sm != 0) {
			cout << "Сумма отрицательных элементов, кратных 2: " << sm << endl;
		}
		else {
			cout << "Сумма отрицательных элементов, кратных 2, не найдена (таких элементов нет) " << endl;
		}
	}
}