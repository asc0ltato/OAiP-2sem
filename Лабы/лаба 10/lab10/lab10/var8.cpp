...
//Написать функцию удаления вершины с максимальной суммой 2 целых значений узла. 
...
{
	int key;         //ключ
	int data1, data2;	// 2 целых числа
	Tree* Left, * Right; // два указателя на потомков
};
...
void main()
{
	...
	for (;;)
	{
		...
		cout << "7. Удалить вершину с максимальной суммой 2 целых значений узла\n";
		cout << "8. Выход\n";
		cout << "Ваш выбор: ";
		cin >> choice;
		cout << "\n";
		...
		case 2:  int data1, data2;
			cout << "\nВведите ключ и два числа: ";
			cin >> key >> data1 >> data2;
...
// Функция поиска узла с максимальной суммой значений
void searchmaxi(Tree* Root) {
	if (maxi <= Root->data1 + Root->data2) // проверка на то, что сумма значений текущего узла <= значению maxi
	{
		maxi = Root->data1 + Root->data2; //Значение переменной обновляется значением суммы текущего узла	
		Root1 = Root;
	}
	if (Root->Left) { // проверяем, есть ли у узла левый дочерний элемент. если да, то рекурсивно вызываем функцию для этого дочернего элемента
		searchmaxi(Root->Left);
	}
	if (Root->Right) { // проверяем, есть ли у узла правый дочерний элемент. если да, то рекурсивно вызываем функцию для этого дочернего элемента
...
void searchmax() {
	...
	searchmaxi(Root);
	Root = delet(Root, Root1->key); // удаление узла с максимальной суммой значений полей из дерева Root
