//Разработать функции работы с приоритетной очередью.Постановка запросов в очередь выполняется по приоритету, снятие - подряд из младших адресов(начало очереди).
//Приоритет: минимальное значение числового параметра, при совпадении параметров - LIFO.
...
int main()
{
	...
	while (choice != 0)
	{
		...
		cout << "7 - извлечь элемент из конца" << endl;
		cout << "0 - выход" << endl;
